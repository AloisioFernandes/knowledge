...
export default {
  ...
  methods: {
    ...
    getArticles() {
      ...
      axios(url).then(res => {
        this.articles = this.articles.concat(res.data) // incrementa articles com resposta da requisição
        this.page++

        if(res.data.length === 0) this.loadMore = false // se não retornar nada da requisição (todos foram carregados)
      })
    }
  },
  watch: {
